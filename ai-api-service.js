...
class HuggingFaceProvider {
    ...
    async generate(prompt, options = {}) {
        ...
        return {
            ...
            model: model,
            provider: this.name,
            tokens: 0 // Hugging Face API不返回token使用量
        };
    }
...
class LocalTemplateProvider {
    constructor() {
        this.name = 'Local Template';
    }
    
    getInfo() {
        return {
            name: this.name,
            description: '本地模板生成，无需网络连接',
            website: '',
            freeTier: true,
            models: ['template']
        };
    ...
    async generate(prompt, options = {}) {
        ...
        // 简化处理，返回一个示例响应
        return {
            content: '这是本地模板生成的周报示例。请配置AI服务以获得更好的生成效果。\n\n' + prompt.substring(0, 200) + '...',
            model: 'template',
            provider: this.name,
            tokens: 0
        };
    ...
    async testConnection() {
        return { success: true, message: '本地模板始终可用' };
    }
}
